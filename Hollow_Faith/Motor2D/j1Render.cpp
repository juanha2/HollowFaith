...
bool j1Render::Awake(pugi::xml_node& config)
{
	...
	else
	{
		...
		camera.x = config.child("camera_pos").attribute("x").as_float();
...
// Blit to screen
bool j1Render::Blit(SDL_Texture* texture, int x, int y, const SDL_Rect* section, float speed_x, float speed_y, SDL_RendererFlip flip, double angle, int pivot_x, int pivot_y) const
{
	bool ret = true;
	uint scale = App->win->GetScale();

	SDL_Rect rect;
	rect.x = (int)(camera.x * speed_x) + x * scale;
	rect.y = (int)(camera.y * speed_y) + y * scale;
